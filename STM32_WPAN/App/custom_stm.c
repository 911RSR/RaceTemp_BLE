...
/* Private typedef -----------------------------------------------------------*/
typedef struct{
  uint16_t  CustomRacechronoHdle;                    /**< RaceChrono handle */
  uint16_t  CustomCan_MainHdle;                  /**< CAN_main handle */
  uint16_t  CustomCan_FilterHdle;                  /**< CAN_filter handle */
...
#define COPY_RACECHRONO_UUID(uuid_struct)          COPY_UUID_128(uuid_struct,0x00,0x00,0x1f,0xf8,0x00,0x00,0x10,0x00,0x80,0x00,0x00,0x80,0x5f,0x9b,0x34,0xfb)
    ...
    case HCI_VENDOR_SPECIFIC_DEBUG_EVT_CODE:
      ...
      switch (blecore_evt->ecode)
      {
        case ACI_GATT_ATTRIBUTE_MODIFIED_VSEVT_CODE:
          ...
          if (attribute_modified->Attr_Handle == (CustomContext.CustomCan_MainHdle + CHARACTERISTIC_DESCRIPTOR_ATTRIBUTE_OFFSET))
          {
            ...
            switch (attribute_modified->Attr_Data[0])
            {
              ...
              case (!(COMSVC_Notification)):
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_CAN_MAIN_NOTIFY_DISABLED_EVT;
              ...
              case COMSVC_Notification:
                ...
                Notification.Custom_Evt_Opcode = CUSTOM_STM_CAN_MAIN_NOTIFY_ENABLED_EVT;
                Custom_STM_App_Notification(&Notification);
          ...
          else if (attribute_modified->Attr_Handle == (CustomContext.CustomCan_FilterHdle + CHARACTERISTIC_VALUE_ATTRIBUTE_OFFSET))
          {
            return_value = SVCCTL_EvtAckFlowEnable;
...
void SVCCTL_InitCustomSvc(void)
{
   ...
   * service_max_attribute_record = 1 for RaceChrono +
   *                                2 for CAN_main +
   *                                2 for CAN_filter +
   *                                1 for CAN_main configuration descriptor +
   *                              = 6
   *
  ...
  COPY_RACECHRONO_UUID(uuid.Char_UUID_128);
  ret = aci_gatt_add_service(UUID_TYPE_128,
                             (Service_UUID_t *) &uuid,
                             PRIMARY_SERVICE,
                             max_attr_record,
                             &(CustomContext.CustomRacechronoHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_service command: RaceChrono, error code: 0x%x \n\r", ret);
  }
  else
  {
  ...
  ret = aci_gatt_add_char(CustomContext.CustomRacechronoHdle,
                          UUID_TYPE_16, &uuid,
                          SizeCan_Main,
                          ...
                          &(CustomContext.CustomCan_MainHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CAN_MAIN, error code: 0x%x \n\r", ret);
  }
  else
  {
  ...
  ret = aci_gatt_add_char(CustomContext.CustomRacechronoHdle,
                          ...
                          0x10,
                          CHAR_VALUE_LEN_VARIABLE,
                          &(CustomContext.CustomCan_FilterHdle));
  if (ret != BLE_STATUS_SUCCESS)
  {
    APP_DBG_MSG("  Fail   : aci_gatt_add_char command   : CAN_FILTER, error code: 0x%x \n\r", ret);
  }
  else
  {
...
tBleStatus Custom_STM_App_Update_Char(Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CAN_MAIN:
      ret = aci_gatt_update_char_value(CustomContext.CustomRacechronoHdle,
                                       CustomContext.CustomCan_MainHdle,
                                       0, /* charValOffset */
                                       SizeCan_Main, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CAN_MAIN command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CAN_MAIN command\n\r");
      }
    ...
    case CUSTOM_STM_CAN_FILTER:
      ret = aci_gatt_update_char_value(CustomContext.CustomRacechronoHdle,
                                       CustomContext.CustomCan_FilterHdle,
                                       0, /* charValOffset */
                                       SizeCan_Filter, /* charValueLen */
                                       (uint8_t *)  pPayload);
      if (ret != BLE_STATUS_SUCCESS)
      {
        APP_DBG_MSG("  Fail   : aci_gatt_update_char_value CAN_FILTER command, result : 0x%x \n\r", ret);
      }
      else
      {
        APP_DBG_MSG("  Success: aci_gatt_update_char_value CAN_FILTER command\n\r");
      }
...
tBleStatus Custom_STM_App_Update_Char_Ext(uint16_t Connection_Handle, Custom_STM_Char_Opcode_t CharOpcode, uint8_t *pPayload)
{
  ...
  switch (CharOpcode)
  {
    ...
    case CUSTOM_STM_CAN_MAIN:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomRacechronoHdle, CustomContext.CustomCan_MainHdle, SizeCan_Main, pPayload);
    ...
    case CUSTOM_STM_CAN_FILTER:
      ...
      ret = Generic_STM_App_Update_Char_Ext(Connection_Handle, CustomContext.CustomRacechronoHdle, CustomContext.CustomCan_FilterHdle, SizeCan_Filter, pPayload);
